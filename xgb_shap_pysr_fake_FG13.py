...
#### ============================================================== ####
#### ---------- FUNCTIONS FOR CREATING FAKE FG13 DATASET ---------- ####
#### ============================================================== ####
    
def create_fake_FG13_dataset(df, pixel_width=750*u.pc, output_log_space=True):
    ...
    fake_variable     = np.squeeze(np.random.rand(1, n_points)) * u.M_sun/u.pc**2 #fake units similar to Sigma_gas to try to "trick" the algorithm?
    FG13_var_noise    = list(map(sum, zip(FG13_var, noise_array)))
    FG13_var_noise.append(fake_variable)
    FG13_var.append(fake_variable)
...
#### ======================================================== ####
def make_shap_plot(X, y, test_feature_name="\\log~Sigma_mol", picklefile="XBG_model_shap.pickle", pdf_savefile="shap_summary_plot_FAKE.pdf", log=False):
    ...
    make_math_FUNC = lambda x: f"${x.replace('fake', 'noise')}$"   

    y_label     = make_math_FUNC((y_test.keys())[0])
    #y_label = y.keys()[0] #temporary fix
   

    replace_FUNC      = lambda s: s.replace("fake", "noise")
...
if __name__ == "__main__":
    ...
    [Q_logZsfrFG13_var, Q_logZsfrFG13_var_noise, Q_logZsfrFG13, Q_logZsfr] = create_fake_FG13_dataset(df_all_params_z0_z0pt5)

    
    #df_logZsfrFG13_clean, df_logZsfr_clean = clean_dataframes(df_logZsfrFG13, df_logZsfr)
    
    #_               = make_shap_plot(df_ZsfrFG13_var,       df_ZsfrFG13, picklefile="xgb_shap_FAKE_FG13.pickle",       pdf_savefile="shap_summary_plot_FAKE_FG13.pdf")
    picklefile =  savepath+"xgb_shap_FAKE_FG13_noise.pickle"#_units.pickle"
    if (os.path.isfile(picklefile)==True):
        with open(picklefile, "rb") as f:
            xbg_save_object     = pickle.load(f)
    else:
        xbg_save_object = make_shap_plot(Q_logZsfrFG13_var_noise, Q_logZsfrFG13, picklefile=picklefile, pdf_savefile=savepath+"shap_summary_plot_FAKE_FG13_noise.pdf")
    ...
    #n_epochs        = 10
    eqns_picklefile = savepath + "FAKE_FG13_FOUND_EQNS_"+str(n_epochs)+"EPOCHS.pickle"
