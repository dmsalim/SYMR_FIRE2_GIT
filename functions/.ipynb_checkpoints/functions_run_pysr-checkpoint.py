...
def make_seperate_loss_curves(ax_FULL_loss_train, ax_FULL_loss_test, ax_QUANT_loss_train, ax_QUANT_loss_test, df_top_eqns, 
    ...
    ylim_FUNC    = lambda arr, pad=0.1: [np.nanmin(arr)-pad, np.nanmax(arr)+pad]
    FULL_ylim    = ylim_FUNC([df_train_FULL_loss_top_eqns.values,     df_test_FULL_loss_top_eqns.values])    #[0.262, 0.32]
    QUANT_ylim   = ylim_FUNC([df_train_QUANTILE_loss_top_eqns.values, df_test_QUANTILE_loss_top_eqns.values])#[0.00, 0.049]
    ...
    ax_QUANT_loss_test.legend(ncol=2, loc="lower right", fontsize=8.5)
